...
% created between 2021-05-18 and 2021-06-15), have false timestamp indices
% ("data_aux.parts).
...
if myIsField(manifest_struct,'data_aux') && numel(manifest_struct.data_aux) > 1
    ...
    %     cmd = "rename " + '"' + manifest_file + '" "' + ...
    %     fullfile(fold,('original_mani'+ext)) + '"';
    %     system(cmd);
    movefile(manifest_file,fullfile(fold,strcat('original_mani',ext)))
    %     testfile = fullfile(fold,strcat('manifold_test',ext));
