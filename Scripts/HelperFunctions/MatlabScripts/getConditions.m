...
% Consider the following additional Triggers: (1) Continuous Bodyspeed / Accelleration
...
%         fs = str2double(fsString{1});
%         if isnan(fs)
%             fprintf('Input should only consist of numbers\n')
%             fprintf('No output file written\n')
%             return
%         end
%     catch
...
%         return
%     end
%     save(fullfile(intanDir,[rhdName, '_sampling_frequency.mat']),...
%         'fs')
% end
...
FileInfo = dir(rhdFile);
fileDate = datetime(FileInfo.date,'Locale','de_DE');
% The first sessions with the apertures were conducted on 2021-09-15.
% Use a different WhiskerContact script for the succeeding sessions.
apertureDate = datetime(2021,09,14);

if fileDate > apertureDate
    HispeedTrials = getWhiskerContactsApertures(sessionDir);
else
    HispeedTrials = getWhiskerContacts(sessionDir);
end
...
%         whiskerLeftOnset(i) = HispeedTrials.ContactLeft{i}(1)*fac;
...
%                 whiskerLeftOffset(i) = HispeedTrials.ContactLeft{i}(m)*fac;
                break
            end
        end
    end
...
%         whiskerRightOnset(i) = HispeedTrials.ContactRight{i}(1)*fac;
...
%                 whiskerRightOffset(i) = HispeedTrials.ContactRight{i}(m)*fac;
                break
            end
        end
    end
...
% Consider the following additional Conditions: (1) WhiskerContact_reward
% (2) WhiskerContact_neutral (3) WhiskerContact_punish (4) Locomotion
% (5) Quiscence
