function [scat, ContourPatch] = kscontour(X, varargin)
...
if iscell(X)
    ContourPatch = nan(1, length(X)); %handles for each
    for iX = 1:length(X)
        ContourPatch(iX) = kscontour(X{iX}, 'Color', colors(mod(iX-1,8)+1,:), ...
...
while iC<size(C,2)
    ...
    ContourPatch = patch(C(1,ix), C(2,ix), max(0, (Color*256-2*tC)/256), ...
...
% Show raw points?
if ShowPoints
%     scatter(X(:,1), X(:,2), PointSize, 'pentagram','filled' , ...
%             'MarkerFaceColor', Color);
    scat = scatter(X(:,1), X(:,2), PointSize, 'filled' , ...
            'MarkerFaceColor', Color);
end
