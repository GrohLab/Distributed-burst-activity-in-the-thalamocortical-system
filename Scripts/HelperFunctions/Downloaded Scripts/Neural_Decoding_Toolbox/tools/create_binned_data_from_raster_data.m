function saved_binned_data_file_name = create_binned_data_from_raster_data(raster_file_expression, save_prefix_name, bin_width, sampling_interval, start_time, end_time)
...
if iscell(raster_file_expression)
    % If the input is a cell array of files, use those
    rasterFiles_select = raster_file_expression;
    rasterDir_name = fileparts(raster_file_expression{1});
elseif contains(raster_file_expression, '*')
    ...
    raster_file_dir = dir(raster_file_expression);
    rasterFiles_select = fullfile({raster_file_dir.folder},{raster_file_dir.name})';
    rasterDir_name = fileparts(raster_file_expression);
else
    raster_file_dir = dir(fullfile(raster_file_expression,'*raster_data.mat'));
    rasterFiles_select = fullfile({raster_file_dir.folder},{raster_file_dir.name})';
    rasterDir_name = fileparts(raster_file_expression);
end


if isempty(rasterFiles_select)
    if ~exist(rasterDir_name,'dir')
...
if nargin < 6
    load(rasterFiles_select{1},'raster_data');
...
for i = 1:length(rasterFiles_select)

    % print a message the the data is being binned (and add a dot for each file that has been binned
    curr_bin_string = [' Binning the data: ' num2str(i) ' of ' num2str(length(rasterFiles_select))];
    if i == 1
   ...
   load(rasterFiles_select{i},'raster_data','raster_labels','raster_site_info');  
...
binned_site_info.binning_parameters.raster_file_directory_name = raster_file_expression;
